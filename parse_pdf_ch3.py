...
pdf_text = user_inputs[-1]['content']
...
for line in lines:
    ...
    # Ignore headers like Basic Level, TO P I C, etc.
    ...
    # Number followed by space (no dot) -> e.g. "1 What is the primary..."
    m_q = re.match(r'^(\d+)\s+(.*)', line)
    if m_q and len(m_q.group(2)) > 5 and not "Topics " in line:
        # Check if it's really a question by looking if previous is finished
        if current_question:
    ...
    if current_question:
        ...
        if m_opt and len(line) > 2 and not current_question.get("explanation"):
            ...
            # Additional check: If option is 'A', we should make sure we don't have 'A' already.
            # Avoid misidentifying lines starting with A as options
            # Only match if previous option is the previous letter or it's A and we have 0 options
